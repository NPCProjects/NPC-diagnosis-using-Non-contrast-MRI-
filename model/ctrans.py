...
class CTrans1(nn.Module):
    def __init__(self, embedding_dim=512, dim=8, depth=1, heads=8, mlp_dim=3072, dropout_rate=0.1, n_levels=1,
                 n_points=4):
...
class CMF(nn.Module):
    def __init__(self, embedding_dim, dim):
        ...
        # self.decode_conv = nn.Conv3d(embedding_dim * 3, 96, kernel_size=1, padding=0) # Ablation Study remove module 2
    ...
    def forward(self, x1, x2, x3, pos1, pos2, pos3):
        ...
        out = out_token.view(out_token.size(0), 8, 8, 8, self.transformer_basic_dims * 3).permute(0, 4, 1, 2,
                                                                                                  3).contiguous()
        out = self.decode_conv(out)
        return out + convpass_out
        # return out
        # return convpass_out # revision
...
if __name__ == '__main__':
    ...
    # model = CMF(embedding_dim=128, dim=8) # Ablation  Remove Module 2
